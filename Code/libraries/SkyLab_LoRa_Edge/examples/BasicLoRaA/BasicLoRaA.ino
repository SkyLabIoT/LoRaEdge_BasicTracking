...
bool motionActivation = 1;  //activation for motionfrequency

#define MAX_GNSS_SCANS_PER_INTERVAL 3 //max retries when not enough sats are found
#define MIN_GNSS_SAT_NEEDED 5 //currently a min of 5 is needed
#define MAX_GNSS_SAT_NEEDED 6 //max sats to scan for
#define MINUTES_AFTER_FAILED_JOIN_TO_REJOIN 10 //minutes to sleep between join retries

byte acc_threshold = 0b00000111; //motion detection threshold
byte acc_duration = 0b00000010; //motion detection duration
...
#define CONFIG_PAYLOAD_FORMAT 4
  ...
  if (joiningFailed == 1)
  {
    ...
    if (beaconActivation == 1) {
      ...
      if (counter >= intervalCurrent) {
        ...
        lr1110_modem_get_gps_time(&timeCurrent);
...
uint8_t send_payload(uint8_t payloadKind) {
  ...
  if (payloadKind == CONFIG_PAYLOAD_FORMAT) {
    ...
    if (motionActivation != 0) {
      tx_frame_buffer[6] = motionIntervalTime;
    }
    tx_frame_buffer[7] = timeInMotionInterval;
    tx_frame_buffer[8] = acc_threshold;
    tx_frame_buffer[9] = acc_duration;
    modem_response_code = lr1110_modem_request_tx(44, is_tx_confirmed, tx_frame_buffer, 10 );//send over port 44, ack defined above, send the buffer, size of payload bytes
    if (modem_response_code != 0) {   //error check
      Serial.print("SEND ERROR: ");
      Serial.println(modem_response_code, HEX);
      return 2;     //return 2 for SEND ERROR
...
void motion_detect() {
  if (beaconActivation != 1 && joiningFailed != 1) {
    ...
    intervalCurrent = motionIntervalTime;
    motionCountDown = timeInMotionInterval + 1;
    Serial.println("Motion detected");
  }
}

void read_config_flashEEPROM() {
  if (!EEPROM.isValid()) {
    Serial.println("Emulated EEPROM is empty");
