...
uint32_t pin;
uint16_t intervalTime = 1; //default interval, in minutes
...
/*payload activations, currently only 2 work simultaneously*/
...
void beacon_function() {  //function that activates beacon
  ...
  lr1110_modem_response_code_t modem_response_code;
  modem_response_code = lr1110_modem_test_tx_single(869800000, 22, LR1110_MODEM_TST_MODE_SF7, LR1110_MODEM_TST_MODE_125_KHZ, LR1110_MODEM_TST_MODE_4_5, 51);
...
void main_function() {  //function to send the data
  ...
  if (modemStat != 0x08)
  {
    ...
    Serial.println(modem_response_code);
    LR1110_rejoin();
    counter = (intervalTime); //retry next minute
  }
  else {
    if (WiFiActivation == 1) {
      if (ledActivation == 1) {
...
uint8_t send_payload(uint8_t payloadKind) {
  ...
  if (payloadKind == WIFI_PAYLOAD_FORMAT) {
   ...
   int16_t BMEpressS = BMEpress*10;
    ...
    float LSMaccX = (event.acceleration.x);
    int16_t LSMaccXS = LSMaccX * 10;
    float LSMaccY = (event.acceleration.y);
    int16_t LSMaccYS = LSMaccY * 10;
    float LSMaccZ = (event.acceleration.z);
    int16_t LSMaccZS = LSMaccZ * 10;
    ...
    tx_frame_buffer[27] = ((LSMaccXS)>>8);
    tx_frame_buffer[28] = ((LSMaccXS));
    tx_frame_buffer[29] = ((LSMaccYS)>>8);
    tx_frame_buffer[30] = ((LSMaccYS));
    tx_frame_buffer[31] = ((LSMaccZS)>>8);
    tx_frame_buffer[32] = ((LSMaccZS));
    
    mag.getEvent(&event);
    float LSMmagX = (event.magnetic.x);
    int16_t LSMmagXS = LSMmagX;
    float LSMmagY = (event.magnetic.y);
    int16_t LSMmagYS = LSMmagY;
    float LSMmagZ = (event.magnetic.z);
    int16_t LSMmagZS = LSMmagZ;
    Serial.print("Magneto x: ");     
    Serial.println(LSMmagX);
    Serial.print("Magneto y: ");     
    Serial.println(LSMmagY);
    Serial.print("Magneto z: ");     
    Serial.println(LSMmagZ);
    
    tx_frame_buffer[33] = ((LSMmagXS)>>8);
    tx_frame_buffer[34] = ((LSMmagXS));
    tx_frame_buffer[35] = ((LSMmagYS)>>8);
    tx_frame_buffer[36] = ((LSMmagYS));
    tx_frame_buffer[37] = ((LSMmagZS)>>8);
    tx_frame_buffer[38] = ((LSMmagZS));
    LSM303_sleep();
    Wire.end();*/
    modem_response_code = lr1110_modem_request_tx(2, is_tx_confirmed, tx_frame_buffer, 27); //send over port 2, ack defined above, send the buffer, 27 bytes
...
void LR1110_event_flush() { //function flushes the open events in the LR1110 and reads the downlink if the event is of the donwlink type
  ...
  while ( digitalRead (eventPin) == 1 ) //only do something when eventpin is high and keep doing until pin goes low
  {
    ...
    if (event_fields.event_type == LR1110_MODEM_LORAWAN_EVENT_DOWN_DATA) {  //if eventtype is down data do things with the received data
      ...
      if (event_fields.buffer[6] == 0) {  //byte 0 contains WiFiActivation, set WiFiACtivation variable accordingly, or do noting when value is not 0 or 1
        WiFiActivation = 0;
      }
      if (event_fields.buffer[6] == 1) {
        WiFiActivation = 1;
      }
      if (event_fields.buffer[7] == 0) {  //byte 0 contains GNSSActivation, set GNSSActivation variable accordingly, or do noting when value is not 0 or 1
        GNSSActivation = 0;
      }
      if (event_fields.buffer[7] == 1) {
        GNSSActivation = 1;
      }
...
void LSM303_reinit(){
  accel.begin();
  accel.setRange(LSM303_RANGE_4G);
  accel.setMode(LSM303_MODE_LOW_POWER); //wake accelerometer
  Wire.beginTransmission(byte(0b0011110));  //wake magento sensor
