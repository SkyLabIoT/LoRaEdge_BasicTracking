...
uint32_t pin;
uint16_t intervalTime = 5; //default interval, in inutes
uint16_t controlTime = 1415; // defauly control interval, in minutes, control message in a bit less than every 24h
bool ledActivation = 1; //activation for activity LED
bool GNSSActivation = 1;  //activation for GNSS scan
bool WiFiActivation = 1;  //activation for WiFi scan
...
void loop() {//puts uC in sleepmode when nothing to do, remove these 2 commands if USBserial debugging is needed
  USBDevice.detach();
  LowPower.deepSleep();
}
